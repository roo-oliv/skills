# Fixer — addresses the consolidated review
...
2. **Triage.** For each assigned finding (every Blocker, High and Medium of the posted review):
   ...
   - Opportunistic cleanup: if a Medium lives in a file you're already editing for a High, fix it along the way.
...
3. **Verify.** Run the repo's **incremental Verify** command (config › Verify — format, lint, tests scoped to the touched files), **always appending the always-run gates** listed in config › Verify › always-run gates (they're cheap and are exactly the gates that fail CI otherwise). The **full** Verify command stays with the PR's CI, which is the authoritative gate and is watched by the orchestrator after the push. If config has no Verify section, ask the orchestrator for the format/lint/test command rather than guessing.
4. **Commit + push.** Use the commit/PR language and conventions from config › Conventions (Conventional Commits if enabled, `fix(scope): ...` referencing the review — `addresses the consolidated review`). Append the commit trailer from config › Conventions if one is listed, **and the `--trailer "Premises-Violated: …"` string the prompt hands you, verbatim, on every commit of this fix** — the ids come from the assigned findings, not from your assessment, and `violatedTrailer` in your output says whether every commit carries it. `git push origin <branch>` — never force, never main.
...
A fix that changes a **predicate, temporal window, formula, or the semantics of a load-bearing value** is not just code — it re-specifies the design. The fix-review and every future reviewer (the repo's CI review, the human code owner, a later run of this loop) receive the branch's plan-contract as the current spec; a fix that doesn't update it makes the reviewer re-derive (or ignore) the semantics you minted. In the audited run each round attacked the previous round's resolution, at ~2M tokens per facet. In the same commit as the fix:
...
- `commits`: pushed SHAs
- `prBodyUpdated`: bool
- `violatedTrailer`: bool — true iff every commit you created carries the `Premises-Violated` trailer the prompt handed over (false when it asked for none)
