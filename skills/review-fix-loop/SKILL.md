...
description: "Review→fix loop in clean context over an open PR: review (mirror of Anthropic's code-review action, or deep-review's lenses + facet enumeration when the change is complex / sensitive-domain / deep-planned), conciliate with comments and reviews already posted on the PR, post the consolidated review to GitHub, and a fixer agent addresses the points (commit+push, updates the description, replies when it diverges). Round 1 breadth, later rounds depth (validator + fix-diff + re-enumeration); repeats until exhaustion — 0 High/Blocker in a breadth round with a dry enumeration — plus a final Medium/Low round. Cadence per profile: direct (cap 3 rounds, scoped gate) or thorough (cap 5, full gate)."
argument-hint: "[PR number | URL | empty = current branch's PR] [deep|standard] [profile=direct|thorough] [max-rounds=N]"
...
- `profile=direct|thorough` → cadence profile (see Contract). **Pick `direct` by default** — `thorough`'s defaults were calibrated for models whose individual passes are less reliable, where confirmation redundancy pays more. Use `thorough` when: the PR is large/multi-surface (>~25 production files), the plan did NOT go through refutation (no deep-plan contract on the branch), or the user asked for maximum exhaustiveness. A user cadence directive ("fewer rounds", "be direct", "be thorough") maps to the profile — not to prose in the args.
- `max-rounds=N` → override the cap (default per profile: **3** direct / **5** thorough).
- `max-extensions=N` → override the dynamic cap extensions (default per profile: **1** direct / **2** thorough; see Contract).
...
`sensitiveDomains` is the subset of touched domains that the config marks sensitive (empty if none). `flows` is the set of flow docs the change touches, copied from the flows dir (`[{ name, doc }]` where `doc` is the flow doc's full text, empty if none) — in deep mode the workflow runs the universal lenses always, plus one `flow-lens` agent per flow. `maxRounds` / `maxExtensions` / `maxEnumSurfaces` are optional — omit them to use the profile defaults (direct: 3/1/4; thorough: 5/2/6); pass them only as a deliberate override. Runs in the background; wait for the `<task-notification>`.
...
Always include: finding counts by severity per round **with the round type and the enumeration** (the trajectory — `breadth 7 → depth 3 (non-dry enum) → gate 0 (dry)` tells the story; a jump of Blockers in a late round signals a fix-regression), documented divergences (points where the fixer disagreed with the reviewer and replied on the PR), whether `gateNeverRan` (exhaustion not confirmed — recommend the manual gate via `/deep-review`), **non-empty `lensFailures`** (lenses that crashed in some breadth/gate round — require the confirmation pass above before trusting a `clean`/`clean-partial`), and whether the mode was standard or deep and why. With `profile: "direct"`, if the gate ran **scoped** (the gate round's `lenses` field is smaller than the full lens set), say which lens set exhaustion was confirmed over — it is a deliberate cadence choice, and the user needs to see it to be able to ask for the full gate.
...
- **Cadence profile (`profile`)**: `thorough` (the workflow default when the arg is omitted — the historical behavior, calibrated for models whose individual passes are less reliable) vs `direct` (strong passes). Calibration from a real production run of this pipeline (a single-seam sensitive-domain bugfix with a refuted plan-contract): the review loop consumed ~78% of the pipeline's subagent tokens even with manually reduced caps, and after a full round 1 found all 3 Highs, rounds 2–3 found ZERO new ones — the value concentrates at the start, the confirmation redundancy at the end. The profile sets the `maxRounds`/`maxExtensions`/`maxEnumSurfaces` defaults (3/1/4 vs 5/2/6), the **scoped gate**, and the **terminal verify** policy (below); explicit overrides always win. The result echoes `profile` for the report.
...
- **Termination by exhaustion**: clean requires 0 High/Blocker **in a breadth round** (round 1 or gate) **with a dry enumeration**. A depth round that zeroes H/B only schedules the **fresh-eyes gate** the following round — quiet is not exhausted; the first-seen curve of the audited run debuted 7 Blocker/High in round 4. The gate earns +1 round beyond the cap (once) if needed. Once exhaustion is confirmed → **one last fix round** for the pending Medium/Low and **no new review after it**.
- **Scoped gate (`profile: "direct"`)**: the fresh-eyes gate is the loop's single most expensive purchase (~1/3 of the cost on a clean run) and re-pays coverage the no-signal lenses already gave in round 1. Under `direct`, when **no Blocker** appeared in the loop, the gate re-runs only the **lenses that produced findings in breadth rounds + negative-space + contract×code** — the declared exhaustion covers that set (logged, and echoed in the gate round's `lenses` field; a deliberate cadence decision, not a silent shortcut). The full fan-out returns automatically if any Blocker was seen in the loop (a fix regression is exactly when full fresh eyes pay) and is always used under `thorough`.
- **Terminal verify per profile**: under `thorough`, the fixer of the terminal push (the final Medium/Low round, or round == effective cap) runs the full **Verify** command locally — any terminal push of the loop leaves fully verified. Under `direct`, it runs the **targeted** verify (format/lint + the tests covering the branch diff + the always-run gates from config › Verify) and the full build stays with the **PR's CI** — the authoritative gate anyway, whose check Step 4 already mandates (`gh pr checks --watch`). CI is mandatory under both profiles; what changes is not paying for the full build twice.
