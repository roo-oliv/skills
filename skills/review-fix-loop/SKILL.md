...
- `status: "done"` — the structure completed. Report the exact trajectory (`scope → breadth (N lenses) → judge → review posted → fix → fix-review [→ fix-2 → fix-review-2]`), `invocations`, the link to the posted review and to the fix-review comment, `severities`, `acceptance` (posted → addressed by a commit) and `openHighBlocker` — Blocker/High that survived the last fix-review **do not stop the loop**: say they were left to the CI review and the human reviewer.
...
- **3. Conciliate and post** — one agent fuses the judged review with what is already on the PR (humans, bots), dedups, does not re-raise a finding contested with an argument, and posts the consolidated review (`gh pr comment` — the GitHub API rejects `gh pr review` on one's own PR).
