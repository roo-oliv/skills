...
- Append `full` to run every agent on the decision tier; the default is the tiered `economy` routing below (`full` also switches on by itself when the branch carries a plan-contract).
...
Every `agent()` call declares `model` and `effort` from the `ROLE` table at the top of
`.claude/workflows/deep-plan.js` — **the `.js` is the executable source of truth**; this table is a
pointer to it. Roles map onto two tiers a repo may override verbatim in `docs/agents/skills-config.md`
› **Models**: `decision` (default `opus`/`high`) and `worker` (default `sonnet`/`medium`).

| Agent | Tier | Reason |
|---|---|---|
| 1 — Dimension Table | **decision** | Multi-stage reasoning; base/cap correctness |
| 2 — Caller-Enumerated Matrix Columns | **worker** | Structured grep + enumerate pattern |
| 3 — State-Mutation Seams | **decision** | Complex multi-path mutation logic |
| 4 — Matrix Cells / Lifecycle | **worker** | Sequential lifecycle against an explicit checklist |
| 5 — Failing-First Tests & Premises | **worker** | Semi-structured obligation analysis |
| 6 — Refuter | **decision** | Adversarial; independence is the whole value |
| Resolver (fill · refute-resolve · gate-justify) | **decision** | Writes the contract that ships |
| Consolidator (Phase 6 narrative) | **decision** @ medium | Narrow rubric, not a search |

`economy` is the default tier; `full` (by the `full` token, or by the sensitive-domain / plan-contract
trigger) upgrades agents 2, 4 and 5 to the decision tier too. **Thinking is never disabled — effort is
lowered instead**, and agents of the same phase share model/effort/schema so they share a cache prefix.
**Never set `CLAUDE_CODE_SUBAGENT_MODEL`**: it is first in the model-resolution order, so it overrides
the whole table and collapses the tiering.
...
|---|---|---|
| 1 — Dimension Table | `agents/dimension-table.md` | Artifact 4: variable→unit/base→cap→seam (a currency amount is the canonical kind; any load-bearing derived value qualifies) |
...
## Phase 4: REFUTE (heavy pass) — one breadth round

Agent 6 (the refuter) gets **only the draft contract**, not the reasoning that produced it — anchoring-free. Its job: find one unhandled scenario, one dimension/cap violation, or one wrongly-`handled` cell. Refute-or-promote. Each refutation carries an **`attacks` tag** — `resolution` (it breaks a fill/fix the draft already contains) vs `new-surface` (it names territory the draft lacks) — which the verdict aggregates to tell fix-attack equilibrium apart from undiscovered surface. The Workflow runs **`REFUTERS_PER_ROUND` refuters in parallel** (each assigned a **distinct attack lens** — quantity/dimensional · async-ordering/lifecycle · premise/invariant · completeness/wrong-cell — plus the list of already-open roots, so the N refuters cover N near-independent surfaces instead of colliding on the same targets or re-raising a known GAP); each surfaced gap reopens the relevant cell/row and the resolver integrates it. Independence is the point — it breaks the cascade where planner and reviewer share a blind spot.

**One round, not a loop.** Every recorded trajectory is FLAT: rounds 2–3 mint as many fresh refutations as round 1, and their `attacks` mix shows most of them attacking the *previous round's resolutions* — fix-attack equilibrium, not discovery. So depth is 1 by default (`REFUTE_ROUNDS`, `ARGS.refuteRounds` opts into more for a single run) and coverage comes from **breadth** (the lensed refuters) and from an **independent re-run** (see *One run does not exhaust*). With one round there is no unattacked-final-patch problem, so there is no re-refute pass either: nothing verifies the resolver's own work in-run — an independent run does.
...
The **heavy (Workflow) pass already emits its own verdict header** — `## deep-plan contract — …` followed by a `### Verdict` block (contradiction count if any, substantive-GAP count + clustering, refute status + trajectory shape + resolution/new-surface mix, reframed gate verdict) and a `### ⚠️ Unresolved` list, all rendered by `renderVerdict()`. Present that body as-is; you only add a one-line **Mode·Tier** banner above it (metadata the Workflow can't know):
...
- **The dimension axis is king.** A load-bearing derived value with no tagged base or no cap seam is a Blocker-class gap, exactly as in deep-review. A currency amount is the canonical case; a count, a window-bounded sum, or a ratio with the wrong base is the same bug.
...
- **Re-audit on model upgrade.** Every constant here — refuter breadth, round depth, which agent gets which tier — encodes what the models of its day did not do alone. On a CLI model upgrade, re-run a baseline plan, compare cost and valid findings, and update `meta.calibratedFor` in the workflow.
...
- **Refute breadth and depth: 4 lensed refuters (`REFUTERS_PER_ROUND`, `ARGS.refutersPerRound` overrides) in ONE round (`REFUTE_ROUNDS`, `ARGS.refuteRounds` opts into depth).** History: a 4×5 run *did* reach the full r1+r2 cluster union in one run, but at ~1.8× cost, still no convergence (flat 16/12/15/15/15), and the 374-violation gate above — because more refuters *amplified* the then-unfixed label drift. Once that amplifier was fixed twice over (`normCode` keying + `canonicalizeMatrixStates`) and a coherent matrix was confirmed, breadth was **re-raised deliberately 2→4** — the post-mortem's own invitation. The lever is *breadth, not depth*: the FLAT `11/11/8` trajectory showed coverage-per-round (the number of distinct attack angles) was the binding constraint, and refuters run in **parallel** so breadth is ~free on wall-clock (it trades tokens for coverage); raising *rounds* (depth) just burns serial wall-clock without converging. To keep added refuters from colliding, each takes a **distinct lens** (quantity/dimension · async-ordering · premise · completeness). The ceiling is the resolver patch size (all fresh funnel into ONE patch → StructuredOutput bloat), not thinking — so it is a band, not "more is better". The refuters are fed the **already-open roots** (GAP cells + unresolved violations) so they push to new territory. For *coverage* prefer an independent re-run over raising rounds — see **One run does not exhaust** below.
...
This is breadth at the *run* level: cheaper and higher-yield than raising **round depth** (`REFUTE_ROUNDS`), which the FLAT trajectory shows never converges. **Breadth** (`REFUTERS_PER_ROUND`, 4 lensed) is the one in-run knob worth raising — it adds near-independent attack surfaces at ~no wall-clock cost — and `ARGS.refutersPerRound` dials it per-run for an unusually large change. But for *coverage* of the finding space, an independent re-run still beats raising any in-run knob; reach for it first, and read the new run's `### Verdict` trajectory shape to decide whether another is warranted.
